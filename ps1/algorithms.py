...
def algorithm1(problem, trace = None):
    ...
    divider = crossProduct(range(problem.numRow), [mid]) #K: returns list like [(1, 3), (2, 3), (3, 3), (4, 3)] if 3 is the number of middle col

    # find the maximum in the dividing column
    bestLoc = problem.getMaximum(divider, trace) #K: returns location (x, y) that has maximum value in column y
    ...
    if neighbor == bestLoc: #K: confirmed works fine, if there is no better neighbor means pean
...
def algorithm3(problem, bestSeen = None, trace = None):
    ...
    subproblems.append((subStartR1, subStartC1, subNumR1, subNumC1)) #K: top left corner
    subproblems.append((subStartR1, subStartC2, subNumR1, subNumC2)) #K: top right corner
    subproblems.append((subStartR2, subStartC1, subNumR2, subNumC1)) #bottom left corner
    subproblems.append((subStartR2, subStartC2, subNumR2, subNumC2)) #bottom right corner
...
def algorithm4(problem, bestSeen = None, rowSplit = True, trace = None):
    ...
    if rowSplit:
        ...
        # information about the two subproblems
        (subStartR1, subNumR1) = (0, mid) #top to mid
        (subStartR2, subNumR2) = (mid + 1, problem.numRow - (mid + 1)) #mid to end
        (subStartC, subNumC) = (0, problem.numCol) #all columns

        subproblems.append((subStartR1, subStartC, subNumR1, subNumC)) #top half of matrix
        subproblems.append((subStartR2, subStartC, subNumR2, subNumC)) #bottom half of matrix

        # get a list of all locations in the dividing column
        divider = crossProduct([mid], range(problem.numCol)) #[(mid, 1), (mid, 2), ..., (mid, problem.numCol)]
    else:
        # the recursive subproblem will involve half the number of columns
        mid = problem.numCol // 2
        ...
        subproblems.append((subStartR, subStartC1, subNumR, subNumC1)) #left half of matrix
        subproblems.append((subStartR, subStartC2, subNumR, subNumC2)) #right half of matrix

        # get a list of all locations in the dividing column
        divider = crossProduct(range(problem.numRow), [mid]) #[(1, mid), (2, mid), ..., (problem.numRow, mid)]
